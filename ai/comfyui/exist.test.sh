...
# subroutines build their workflows from — a stripped or half-started install
# answers 200 and then rejects every /prompt with "node type not found".
...
# One node per comfy subroutine: image (image-text), reference image
# (image-text-image), video (video-i2v).
