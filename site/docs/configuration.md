...
## Endpoints — putting roles on different machines
...
`EXIST_GPU_VENDOR` is `nvidia`, `amd`, `none`, or `external`. It is the **first** question quest asks,
...
| `external` | Same treatment as `none` — this machine has no card to reserve. Ollama is not run locally at all; the models come from `EXIST_OLLAMA_URL` on another machine. |
...
- `EXIST_NTFY_URL` / `EXIST_NTFY_TOKEN` — push notifications. The URL is container-to-container
  (Docker DNS). `EXIST_NTFY_USER` / `EXIST_NTFY_PASSWORD` are the publishing identity, created by
