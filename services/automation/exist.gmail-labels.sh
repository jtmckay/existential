...
#
# Run via: ./existential.sh run automation gmail-labels
# Re-run any time you add a new label in Gmail that you want decree to read.
#
# Requires Gmail credentials (run `./existential.sh run automation gmail-sync` first).
...
[ -f "$CREDENTIALS" ] || die "Gmail credentials not found at ${CREDENTIALS}. Run: ./existential.sh run automation gmail-sync"
...
echo ""
echo "    ./existential.sh run automation gmail-labels"
echo ""
