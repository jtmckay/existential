...
//
// Reads:  services/automation/webhook/config.yml  (endpoints + shared secret)
...
const webhookConfig = yaml.load(
  fs.readFileSync(`${REPO}/services/automation/webhook/config.yml`, "utf-8")
) as WebhookConfig;

const SHARED_SECRET = webhookConfig.secret ?? "";
// Browser-side URL — resolves via pihole DNS through Caddy (no port needed).
const WEBHOOK_BASE = `https://automation-webhook.${DOMAIN}`;
