...
#
# Normally invoked by setup.sh, which sets DECREE_DIR and SECRETS_DIR.
# Can also be run directly:
#   SECRETS_DIR=/path/to/secrets bash .decree/setup/gmail-sync.sh
#
...
if [ -z "${SECRETS_DIR:-}" ]; then
    if [ "${IN_CONTAINER:-}" = "1" ]; then
        ...
        # Script lives at .decree/setup/gmail-sync.sh; secrets is at ../../secrets
        # relative to the daemon/ directory (i.e. services/decree/secrets/)
