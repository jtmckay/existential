...
WEBHOOK="http://decree-webhook:8801"

# ── 1. Rendered state ────────────────────────────────────────────────────────

file_present "webhook config rendered" /repo/services/decree/webhook/config.yml
file_present "decree inbox present" /repo/services/decree/decree/inbox

# ── 2. Webhook liveness ──────────────────────────────────────────────────────

http_probe "decree-webhook /healthz" "${WEBHOOK}/healthz" 200

# Routing coverage — same endpoint through caddy, so "webhook down" stays
# distinguishable from "caddy/pihole routing broken".
probe_caddy "decree-webhook /healthz" decree-webhook /healthz 200
