...
The pipeline:
  You edit workspace/notes/plan.md
    → workspace-sync bisyncs workspace/ with the workspace/ subfolder of the
      nextcloud seaweedfs bucket (cron) — the same bucket Nextcloud mounts at /S3
    → seaweedfs fires a file event → POST http://automation-webhook:8801/s3
...
just workspace/ ones. `s3-router`'s PATTERN match is what keeps that from
...
The loop break — the one thing worth understanding:
  ...
  what an earlier run produced. It is NOT synced to the bucket, so nothing written
  there ever becomes an event. Without that exclusion, every answer would
  trigger another run, forever.

Setup:

  If you ran the Core quest with SeaweedFS, Nextcloud and Decree enabled, most of
  this already happened for you: the bucket, the workspace-sync cron, the
  nextcloud rclone remote, and s3-router/file-processor enabled — Core
  ...
  2. Enable `agent-task` in services/automation/decree/config.yml
     (`s3-router` and `file-processor` are already on by default). The
...
and edit its PATTERN and CRITERIA — no restart needed, s3-router reads the
directory per event.
...
noticed — seaweedfs fires events for objects written through its own API, and edits
