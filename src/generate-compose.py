...
def merge(repo_root: str, enabled: list[str], network_external: bool = False) -> dict:
    services: dict = {}
    volumes: dict = {}
    networks: dict = {}
    ...
    networks['exist'] = {'external': True} if network_external else {'driver': 'bridge'}
...
def merge_env(repo_root: str, enabled: list[str]) -> None:
    """Merge .env.exist and all enabled service .env files into root .env."""
    merged: dict[str, str] = {}

    for path in [os.path.join(repo_root, '.env.exist')] + \
                [os.path.join(repo_root, rel, '.env') for rel in enabled]:
    ...
    with open(env_path, 'w') as f:
        f.write(
            ...
            '# Edit .env.exist (global) or service-level .env files instead\n'
            '# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n'
        )
...
def main() -> None:
    ...
    env = load_env(os.path.join(repo_root, '.env.exist'))
    ...
    network_external = env.get('EXIST_NETWORK_EXTERNAL', 'false').lower() == 'true'
    merged = merge(repo_root, enabled, network_external=network_external)

    merge_env(repo_root, enabled)

    if os.path.exists(output_path):
        ts = int(time.time() * 1000)
        backup = os.path.join(repo_root, f'docker-compose-{ts}.yml')
