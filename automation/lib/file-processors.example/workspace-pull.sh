...
# Workspace pull — the live half of workspace-sync's bucket -> local direction.
#
# workspace-sync (cron, every 10 min by default) covers local -> bucket by
# design: nothing fires when you edit the bind mount by hand, so it has to
# poll. bucket -> local doesn't need to poll — the object store already tells decree about
...
# workspace/ (or deletes it locally on a removed event), so a bucket- or
...
# already set to match the bucket object's — matching modtimes on both sides is
