...
# prompt (no reference image). The prompt text is read from the
# message/spec file. Additional parameters are passed as env vars.
#
# Env vars:
...
if [ "${DECREE_PRE_CHECK:-}" = "true" ]; then
    ...
    command -v curl >/dev/null 2>&1 || precheck_fail "comfy-image-text" "curl not found"
    command -v jq   >/dev/null 2>&1 || precheck_fail "comfy-image-text" "jq not found"
    command -v shuf >/dev/null 2>&1 || precheck_fail "comfy-image-text" "shuf not found"
    [ -f "$(dirname "${BASH_SOURCE[0]}")/../lib/comfy/image_flux2_text_landscape.json" ] \
        || precheck_fail "comfy-image-text" "workflow template image_flux2_text_landscape.json not found"
    precheck_pass "comfy-image-text"
    exit 0
fi
...
TEMPLATE="$SCRIPT_DIR/../lib/comfy/image_flux2_text_landscape.json"
