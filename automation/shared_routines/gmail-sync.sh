...
resolve_label_id() {
    ...
    if [ ! -f "$labels_file" ]; then
        echo "Label cache missing. Run: ./existential.sh run automation gmail-labels" >&2
        exit 1
    fi
    local id
    ...
    if [ -z "$id" ]; then
        echo "Label '${label_name}' not found in cache. Run: ./existential.sh run automation gmail-labels" >&2
        exit 1
    fi
    printf '%s' "$id"
}
...
_handle_auth_failure() {
    if [ ! -f "$_AUTH_FAILURE_FILE" ]; then
        ...
        outfile="${OUTBOX_DIR}/gmail-sync-auth-failure-$(date +%s).md"
        {
            ...
            printf '\n'
            printf 'To reauthorize: ./existential.sh run automation gmail-sync\n'
            printf 'Then restart the daemon: docker compose restart automation\n'
        } > "$outfile"
