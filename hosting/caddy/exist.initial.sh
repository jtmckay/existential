...
# `<slug>.internal` with `tls internal` (Caddy's own CA). Devices that
# imported the root cert get a green padlock; others see a warning.
#
...
# existing `<slug>.internal` blocks remain untouched.
#
# Mechanism: this script parses the live Caddyfile for every `<slug>.internal {
# ... }` block, emits a parallel `<slug>.<domain> { ... }` block (with
# `tls internal` stripped so Caddy uses ACME), writes them all to
...
# Re-run manually: ./existential.sh setup caddy
...
if [ -z "$DOMAIN" ]; then
    ...
    echo "  ✓ Set EXIST_PUBLIC_DOMAIN= (blank) in .env.exist"
    echo ""
...
# Generate Caddyfile.public from the .internal blocks in Caddyfile.
awk -v domain="$DOMAIN" '
    BEGIN {
        print "# Caddyfile.public — generated by hosting/caddy/exist.initial.sh"
        print "# from the .internal blocks in Caddyfile. DO NOT edit by hand;"
        print "# re-run ./existential.sh setup caddy after changing services."
        print ""
        in_block = 0
    }
    /^[a-z][a-z0-9-]*\.internal[[:space:]]*\{[[:space:]]*$/ {
        host = $1
        sub(/\.internal[[:space:]]*\{?[[:space:]]*$/, "", host)
        print host "." domain " {"
        in_block = 1
        next
    }
...
echo "  ✓ Set EXIST_PUBLIC_DOMAIN=${DOMAIN} in .env.exist"
