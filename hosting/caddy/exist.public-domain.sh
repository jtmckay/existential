...
awk -v domain="$DOMAIN" '
    BEGIN {
        print "# Caddyfile.public — generated by hosting/caddy/exist.public-domain.sh"
        print "# from the <slug>.{$CADDY_DOMAIN} blocks in Caddyfile. DO NOT edit by hand;"
        print "# re-run ./existential.sh run caddy public-domain after changing services."
        print ""
...
if ! grep -q '^import Caddyfile\.public$' "$CADDYFILE"; then
    printf '\n# Public-domain blocks (managed by hosting/caddy/exist.public-domain.sh)\nimport Caddyfile.public\n' >> "$CADDYFILE"
